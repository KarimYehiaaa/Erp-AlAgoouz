...
export const menuController = {
  /** قائمة المنيوهات */
  list: wrap(async (_req: Request, res: Response, _next: NextFunction) => {
    ok(res, await menuService.getMenus());
  }),

  /** جلب تفاصيل منيو محدد */
  get: wrap(async (req: Request, res: Response, _next: NextFunction) => {
    ok(res, await menuService.getMenuById(Number(req.params.id)));
  }),

  /** جلب المنيو النشط الحالي */
  getActive: wrap(async (_req: Request, res: Response, _next: NextFunction) => {
    ok(res, await menuService.getActiveMenu());
  }),

  /** إنشاء منيو جديد */
  create: wrap(async (req: Request, res: Response, _next: NextFunction) => {
  ...
  /** تحديث منيو قائم */
  update: wrap(async (req: Request, res: Response, _next: NextFunction) => {
    const userId = (req as any).user?.id || (req as any).user?.userId;
  ...
  /** حذف منيو */
  delete: wrap(async (req: Request, res: Response, _next: NextFunction) => {
    ok(res, await menuService.deleteMenu(Number(req.params.id)), 'تم حذف المنيو بنجاح');
  }),

  /** جلب قائمة المنتجات المتاحة للاختيار السريع */
  getAvailableProducts: wrap(async (_req: Request, res: Response, _next: NextFunction) => {
