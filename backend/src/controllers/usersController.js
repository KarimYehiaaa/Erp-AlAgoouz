...
export const users = {
  ...
  createRole: async (req, res, next) => { try { ok(res, await userService.createRole(req.body), 'تم إنشاء المنصب بنجاح'); } catch (e) { next(e); } },
  updateRole: async (req, res, next) => { try { ok(res, await userService.updateRole(req.params.id, req.body), 'تم تعديل المنصب بنجاح'); } catch (e) { next(e); } },
  deleteRole: async (req, res, next) => { try { ok(res, await userService.deleteRole(req.params.id), 'تم حذف المنصب بنجاح'); } catch (e) { next(e); } },
