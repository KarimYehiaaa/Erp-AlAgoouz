...
export const adminDashboard = {
  ...
  backup: async (req, res, next) => {
    try {
      ...
      res.setHeader('Content-Disposition', `attachment; filename=alagoouz_erp_backup_${new Date().toISOString().slice(0, 10)}.json`);
      res.send(JSON.stringify(backupData, null, 2));
    } catch (e) { next(e); }
  },
  repairSequences: async (req, res, next) => {
    try { ok(res, await systemHealthService.repairSequences(), 'تم إصلاح متسلسلات قاعدة البيانات بنجاح'); } catch (e) { next(e); }
  },
  riskRadar: async (req, res, next) => {
    try { ok(res, await systemHealthService.getRiskRadarReport()); } catch (e) { next(e); }
  },
  purgeLogs: async (req, res, next) => {
    try { ok(res, await systemHealthService.purgeOldAuditLogs(req.body?.days), 'تم تنظيف سجلات النشاط القديمة'); } catch (e) { next(e); }
  },
  setBroadcast: async (req, res, next) => {
    try { ok(res, systemHealthService.setBroadcast(req.body.title, req.body.message, req.body.level), 'تم نشر التنبيه العام للنظام'); } catch (e) { next(e); }
  },
  getBroadcast: async (req, res, next) => {
    try { ok(res, systemHealthService.getBroadcast()); } catch (e) { next(e); }
  },
};
