        ...
        <div class="card">
          ...
          <table class="report-table">
            ...
            <tbody>
              ...
              <tr v-if="!(summary.topProducts||[]).length"><td colspan="3" class="empty">لا توجد بيانات</td></tr>
            </tbody>
          </table>
        </div>
        <div class="card">
          ...
          <table class="report-table">
            <thead><tr><th>المنتج</th><th>المخزون</th><th>الحد الأدنى</th></tr></thead>
            <tbody>
              ...
              <tr v-if="!(summary.lowStock||[]).length"><td colspan="3" class="empty">✅ لا توجد تنبيهات</td></tr>
            </tbody>
          </table>
        </div>
      </div>
    ...
    <template v-if="activeTab === 'pl'">
      ...
      <template v-else-if="plData">
        ...
        <div v-if="plData.cogs_basis === 'purchases'" class="pl-notice card">
          💡 <strong>ملاحظة:</strong> المبيعات مسجلة كإجمالي يومي بدون تفصيل أصناف، لذا يُستخدم <strong>إجمالي المشتريات</strong> كتقريب لتكلفة البضاعة المباعة.
        </div>
        ...
        <div class="kpi-grid kpi-grid-4">
          ...
          <div class="kpi-card sales">
            ...
            <div class="kpi-body">
              ...
              <div class="kpi-sub">{{ plData.revenue?.count }} عملية · خصم {{ formatMoney(plData.revenue?.discounts) }}</div>
            </div>
          </div>
        ...
        <div class="grid grid-2 mt-4">
          ...
          <div class="card pl-statement">
            ...
            <div class="pl-row" v-for="(val, type) in plData.revenue?.by_type" :key="type">
              <span class="pl-indent">{{ { branch: 'مبيعات فرع', wholesale: 'مبيعات جملة', pos: 'POS' }[type] || type }}</span>
            ...
            <div class="pl-row pl-subtotal" :class="plData.gross_profit?.amount >= 0 ? 'positive' : 'negative'">
            ...
            <div class="pl-row pl-deduct" v-for="exp in plData.operating_expenses?.breakdown" :key="exp.category">
              <span class="pl-indent">{{ exp.category }}</span>
              <span class="pl-amount deduct">({{ formatMoney(exp.total) }})</span>
            </div>
            <div class="pl-row pl-subtotal">
              <span>إجمالي المصاريف</span>
              <span class="pl-amount bold deduct">({{ formatMoney(plData.operating_expenses?.total) }})</span>
            </div>

            <div class="pl-divider pl-divider-double"></div>
            <div class="pl-row pl-net" :class="plData.net_profit?.amount >= 0 ? 'positive' : 'negative'">
          ...
          <div class="card">
            ...
            <div class="pl-statement">
              ...
              <div class="pl-divider pl-divider-double"></div>
              <div class="pl-row pl-net" :class="plData.cash_flow?.closing >= 0 ? 'positive' : 'negative'">
            ...
            <div class="mt-4" v-if="(plData.operating_expenses?.breakdown || []).length">
              ...
              <div v-for="exp in plData.operating_expenses?.breakdown" :key="exp.category" class="expense-bar-row">
                <span class="expense-label">{{ exp.category }}</span>
                <div class="expense-bar-wrap">
                  <div class="expense-bar"
                    :style="{ width: plData.operating_expenses?.total > 0 ? (exp.total / plData.operating_expenses.total * 100).toFixed(1) + '%' : '0%' }">
                  </div>
                </div>
            ...
            <table class="report-table">
              ...
              <tbody>
                <tr v-for="row in plTrend" :key="row.month">
                  ...
                  <td class="money" :class="row.gross_profit >= 0 ? 'profit-pos' : 'profit-neg'">{{ formatMoney(row.gross_profit) }}</td>
                  <td class="money" :class="row.net_profit >= 0 ? 'profit-pos' : 'profit-neg'">{{ formatMoney(row.net_profit) }}</td>
                  <td class="center" :class="row.net_margin >= 0 ? 'profit-pos' : 'profit-neg'">{{ row.net_margin }}%</td>
                </tr>
                <!-- إجمالي الصف -->
                <tr class="pl-total-row">
                  <td><strong>الإجمالي</strong></td>
                  <td class="money bold">{{ formatMoney(plTrend.reduce((s,r)=>s+r.revenue,0)) }}</td>
                  <td class="money muted bold">({{ formatMoney(plTrend.reduce((s,r)=>s+r.cogs,0)) }})</td>
                  <td class="money muted bold">({{ formatMoney(plTrend.reduce((s,r)=>s+r.expenses,0)) }})</td>
                  <td class="money bold" :class="plTrend.reduce((s,r)=>s+r.gross_profit,0)>=0?'profit-pos':'profit-neg'">{{ formatMoney(plTrend.reduce((s,r)=>s+r.gross_profit,0)) }}</td>
                  <td class="money bold" :class="plTrend.reduce((s,r)=>s+r.net_profit,0)>=0?'profit-pos':'profit-neg'">{{ formatMoney(plTrend.reduce((s,r)=>s+r.net_profit,0)) }}</td>
                  <td></td>
                </tr>
              </tbody>
            </table>
      ...
      <div class="card">
        <div class="table-header">
          ...
          <div class="type-filter">
            ...
            <button :class="{ active: salesFilter === 'branch' }" @click="salesFilter = 'branch'">فرع</button>
            <button :class="{ active: salesFilter === 'wholesale' }" @click="salesFilter = 'wholesale'">جملة</button>
          </div>
        </div>
        <div class="table-wrap">
          <table class="report-table">
            <thead><tr><th>التاريخ</th><th>النوع</th><th>العدد</th><th>المبيعات</th><th>التكلفة</th><th>الربح</th><th>الهامش</th></tr></thead>
            <tbody>
              <tr v-for="row in filteredSalesRows" :key="`${row.date}-${row.sale_type}`">
                <td>{{ row.date }}</td>
                <td><span :class="['type-badge', row.sale_type]">{{ saleTypeLabel(row.sale_type) }}</span></td>
                <td class="center">{{ row.count }}</td>
                <td class="money">{{ formatMoney(row.total || 0) }}</td>
                <td class="money muted">{{ formatMoney(row.cost || 0) }}</td>
                <td class="money" :class="row.profit >= 0 ? 'profit-pos' : 'profit-neg'">{{ formatMoney(row.profit || 0) }}</td>
                <td class="center">{{ row.total > 0 ? ((row.profit / row.total) * 100).toFixed(1) : 0 }}%</td>
              </tr>
              <tr v-if="!filteredSalesRows.length"><td colspan="7" class="empty">لا توجد بيانات مبيعات</td></tr>
            </tbody>
          </table>
        </div>
      </div>
    ...
    <template v-if="!loading && activeTab === 'inventory'">
      ...
      <div class="grid grid-2 mt-4">
        <div class="card">
          ...
          <table class="report-table">
            <thead><tr><th>المستودع</th><th>المنتجات</th><th>القيمة</th></tr></thead>
            <tbody>
              <tr v-for="row in reportData.inventory?.warehouseValue || []" :key="row.warehouse_name">
                <td>{{ row.warehouse_name }}</td>
                <td class="center">{{ row.products_count }}</td>
                <td class="money">{{ formatMoney(row.total_value || 0) }}</td>
              </tr>
              <tr v-if="!(reportData.inventory?.warehouseValue||[]).length"><td colspan="3" class="empty">لا توجد بيانات</td></tr>
            </tbody>
          </table>
        </div>
        <div class="card">
          ...
          <table class="report-table">
            ...
            <tbody>
              ...
              <tr v-if="!lowStockProducts.length"><td colspan="3" class="empty">✅ لا توجد تنبيهات</td></tr>
            </tbody>
          </table>
        </div>
      </div>
      <div class="card mt-4">
        ...
        <div class="table-wrap">
          <table class="report-table">
            <thead><tr><th>المنتج</th><th>SKU</th><th>التصنيف</th><th>المخزون</th><th>الحد الأدنى</th><th>سعر الشراء</th><th>الحالة</th></tr></thead>
            <tbody>
              <tr v-for="row in reportData.inventory?.products || []" :key="row.product_id">
                ...
                <td><span :class="['status-badge', row.is_low_stock ? 'danger' : 'success']">{{ row.is_low_stock ? 'منخفض' : 'طبيعي' }}</span></td>
              </tr>
              <tr v-if="!(reportData.inventory?.products||[]).length"><td colspan="7" class="empty">لا توجد بيانات</td></tr>
            </tbody>
          </table>
        </div>
      </div>
    ...
    <template v-if="!loading && activeTab === 'profit'">
      ...
      <div class="grid grid-2 mt-4">
        <div class="card">
          ...
          <div class="table-wrap">
            <table class="report-table">
              <thead><tr><th>التاريخ</th><th>الإيراد</th><th>التكلفة</th><th>الربح</th><th>الهامش</th></tr></thead>
              <tbody>
                ...
                <tr v-if="!(reportData.profit?.daily||[]).length"><td colspan="5" class="empty">لا توجد بيانات</td></tr>
              </tbody>
            </table>
          </div>
        </div>
        <div class="card">
          ...
          <table class="report-table">
            <thead><tr><th>التصنيف</th><th>المنتجات</th><th>الإيراد</th><th>الربح</th></tr></thead>
            <tbody>
              <tr v-for="row in reportData.profit?.byCategory || []" :key="row.category_name">
                ...
                <td class="money" :class="row.net_profit >= 0 ? 'profit-pos' : 'profit-neg'">{{ formatMoney(row.net_profit || 0) }}</td>
              </tr>
              <tr v-if="!(reportData.profit?.byCategory||[]).length"><td colspan="4" class="empty">لا توجد بيانات</td></tr>
            </tbody>
          </table>
        </div>
      </div>
    ...
    <template v-if="!loading && activeTab === 'expenses'">
      ...
      <div class="grid grid-2 mt-4">
        <div class="card">
          <h3>📂 حسب التصنيف</h3>
          <table class="report-table">
            <thead><tr><th>التصنيف</th><th>العدد</th><th>الإجمالي</th><th>النسبة</th></tr></thead>
            <tbody>
              <tr v-for="row in reportData.expenses?.byCategory || []" :key="row.category">
                <td>{{ row.category }}</td>
                <td class="center">{{ row.count }}</td>
                <td class="money">{{ formatMoney(row.total || 0) }}</td>
                <td class="center">{{ expensesTotal > 0 ? ((row.total / expensesTotal) * 100).toFixed(1) : 0 }}%</td>
              </tr>
              <tr v-if="!(reportData.expenses?.byCategory||[]).length"><td colspan="4" class="empty">لا توجد مصروفات</td></tr>
            </tbody>
          </table>
        </div>
        <div class="card">
          <h3>📅 حسب الشهر</h3>
          <table class="report-table">
            <thead><tr><th>الشهر</th><th>العدد</th><th>الإجمالي</th></tr></thead>
            <tbody>
              ...
              <tr v-if="!(reportData.expenses?.monthly||[]).length"><td colspan="3" class="empty">لا توجد بيانات</td></tr>
            </tbody>
          </table>
        </div>
      </div>
      <div class="card mt-4">
        <h3>🕐 آخر المصروفات</h3>
        <table class="report-table">
          <thead><tr><th>العنوان</th><th>التصنيف</th><th>التاريخ</th><th>المبلغ</th></tr></thead>
          <tbody>
            <tr v-for="row in reportData.expenses?.recent || []" :key="`${row.title}-${row.expense_date}`">
              <td>{{ row.title }}</td>
              <td>{{ row.category }}</td>
              <td>{{ row.expense_date }}</td>
              <td class="money">{{ formatMoney(row.amount || 0) }}</td>
            </tr>
            <tr v-if="!(reportData.expenses?.recent||[]).length"><td colspan="4" class="empty">لا توجد بيانات</td></tr>
          </tbody>
        </table>
      </div>
    </template>
      ...
      <div class="kpi-grid kpi-grid-3">
        <div class="kpi-card">
          ...
          <div class="kpi-body">
            ...
            <div class="kpi-value">{{ formatMoney(reportData.purchases?.summary?.total_amount || 0) }}</div>
            <div class="kpi-sub">{{ reportData.purchases?.summary?.invoices_count || 0 }} فاتورة</div>
          </div>
        </div>
        <div class="kpi-card profit">
          <div class="kpi-icon">✅</div>
          <div class="kpi-body">
            ...
            <div class="kpi-value">{{ formatMoney(reportData.purchases?.summary?.paid_amount || 0) }}</div>
          </div>
        </div>
        <div class="kpi-card danger">
          <div class="kpi-icon">⏳</div>
          <div class="kpi-body">
            ...
            <div class="kpi-value">{{ formatMoney(reportData.purchases?.summary?.unpaid_amount || 0) }}</div>
          </div>
        </div>
      </div>
      <div class="grid grid-2 mt-4">
        <div class="card">
          <h3>🏢 حسب المورد</h3>
          <table class="report-table">
            <thead><tr><th>المورد</th><th>الفواتير</th><th>الإجمالي</th><th>المدفوع</th></tr></thead>
            <tbody>
              ...
              <tr v-if="!(reportData.purchases?.bySupplier||[]).length"><td colspan="4" class="empty">لا توجد بيانات</td></tr>
            </tbody>
          </table>
        </div>
        <div class="card">
          ...
          <table class="report-table">
            ...
            <tbody>
              <tr v-for="row in reportData.purchases?.recent || []" :key="row.invoice_number">
                ...
                <td>{{ row.supplier_name }}</td>
                <td class="money">{{ formatMoney(row.total_amount || 0) }}</td>
                <td><span :class="['status-badge', row.status === 'paid' ? 'success' : 'warning']">{{ purchaseStatusLabel(row.status) }}</span></td>
              </tr>
              <tr v-if="!(reportData.purchases?.recent||[]).length"><td colspan="4" class="empty">لا توجد بيانات</td></tr>
            </tbody>
          </table>
        </div>
      </div>
    ...
    <template v-if="!loading && activeTab === 'customers'">
      <div class="kpi-grid kpi-grid-3">
        <div class="kpi-card customers">
          ...
          <div class="kpi-body">
            ...
            <div class="kpi-value">{{ reportData.customers?.topCustomers?.[0]?.name_ar || '—' }}</div>
            <div class="kpi-sub">{{ formatMoney(reportData.customers?.topCustomers?.[0]?.total_spent || 0) }}</div>
          </div>
        </div>
        <div class="kpi-card danger">
          <div class="kpi-icon">📄</div>
      ...
      <div class="grid grid-2 mt-4">
        <div class="card">
          ...
          <table class="report-table">
            <thead><tr><th>العميل</th><th>النوع</th><th>المشتريات</th><th>الإجمالي</th><th>الرصيد</th></tr></thead>
            <tbody>
              <tr v-for="row in reportData.customers?.topCustomers || []" :key="row.name_ar">
                <td>{{ row.name_ar }}</td>
                <td><span :class="['type-badge', row.customer_type]">{{ customerTypeLabel(row.customer_type) }}</span></td>
                <td class="center">{{ row.sales_count }}</td>
                <td class="money">{{ formatMoney(row.total_spent || 0) }}</td>
                <td class="money" :class="row.balance < 0 ? 'profit-neg' : ''">{{ formatMoney(row.balance || 0) }}</td>
              </tr>
              <tr v-if="!(reportData.customers?.topCustomers||[]).length"><td colspan="5" class="empty">لا توجد بيانات</td></tr>
            </tbody>
          </table>
        </div>
        <div class="card">
          ...
          <table class="report-table">
            <thead><tr><th>الفاتورة</th><th>العميل</th><th>المبلغ</th><th>الحالة</th></tr></thead>
            <tbody>
              <tr v-for="row in reportData.customers?.unpaidInvoices || []" :key="row.invoice_number">
                <td class="mono">{{ row.invoice_number }}</td>
                <td>{{ row.customer_name || '—' }}</td>
                <td class="money danger-text">{{ formatMoney(row.total_amount || 0) }}</td>
                <td><span :class="['status-badge', row.payment_status === 'partial' ? 'warning' : 'danger']">{{ paymentStatusLabel(row.payment_status) }}</span></td>
              </tr>
              <tr v-if="!(reportData.customers?.unpaidInvoices||[]).length"><td colspan="4" class="empty">✅ لا توجد فواتير معلقة</td></tr>
            </tbody>
          </table>
        </div>
      </div>
...
const reportData = reactive({
  sales: [], inventory: {}, profit: {}, expenses: {}, purchases: {}, customers: {},
});
...
const setQuick = (range) => {
  ...
  if (range === 'today') {
    filters.from_date = fmt(now); filters.to_date = fmt(now);
  } else if (range === 'week') {
    const d = new Date(now); d.setDate(d.getDate() - 6);
    filters.from_date = fmt(d); filters.to_date = fmt(now);
  } else if (range === 'month') {
...
const purchaseStatusLabel = (s) => ({ paid: 'مدفوع', pending: 'معلق', partial: 'جزئي' }[s] || s || '—');
const paymentStatusLabel = (s) => ({ unpaid: 'غير مدفوع', partial: 'جزئي', paid: 'مدفوع' }[s] || s || '—');
const fmtQty = (v) => { const n = Number(v || 0); return n % 1 === 0 ? n.toLocaleString('en-GB') : n.toFixed(2); };
...
const salesTotal = computed(() => filteredSalesRows.value.reduce((s, r) => s + Number(r.total || 0), 0));
const salesProfit = computed(() => filteredSalesRows.value.reduce((s, r) => s + Number(r.profit || 0), 0));
const salesCount = computed(() => filteredSalesRows.value.reduce((s, r) => s + Number(r.count || 0), 0));

const lowStockProducts = computed(() => (reportData.inventory?.products || []).filter(p => p.is_low_stock));
const lowStockCount = computed(() => lowStockProducts.value.length);
const totalInventoryValue = computed(() => (reportData.inventory?.warehouseValue || []).reduce((s, r) => s + Number(r.total_value || 0), 0));

const profitTotalRevenue = computed(() => (reportData.profit?.daily || []).reduce((s, r) => s + Number(r.revenue || 0), 0));
const profitTotalCost = computed(() => (reportData.profit?.daily || []).reduce((s, r) => s + Number(r.cost || 0), 0));
const profitTotalNet = computed(() => (reportData.profit?.daily || []).reduce((s, r) => s + Number(r.profit || 0), 0));

const expensesTotal = computed(() => (reportData.expenses?.byCategory || []).reduce((s, r) => s + Number(r.total || 0), 0));
const expensesCount = computed(() => (reportData.expenses?.byCategory || []).reduce((s, r) => s + Number(r.count || 0), 0));

const customersTotal = computed(() => (reportData.customers?.topCustomers || []).reduce((s, r) => s + Number(r.total_spent || 0), 0));
...
.pl-indent { padding-right: 12px; color: var(--text); }
.pl-amount { font-weight: 600; color: var(--text-strong); font-variant-numeric: tabular-nums; }
.pl-amount.deduct { color: var(--danger); }
.pl-amount.positive { color: var(--success); }
.pl-amount.bold { font-weight: 800; }
.pl-amount.bold-lg { font-size: 1.1rem; font-weight: 900; }
...
.pl-subtotal {
  ...
  padding: 9px 12px;
  margin: 4px 0;
  &.positive .pl-amount { color: var(--success); }
  &.negative .pl-amount { color: var(--danger); }
}

.pl-deduct { opacity: 0.85; }
...
.pl-net {
  ...
  font-size: 0.95rem;
  font-weight: 800;
  &.positive { background: color-mix(in srgb, var(--success) 10%, var(--bg)); }
  &.negative { background: color-mix(in srgb, var(--danger) 10%, var(--bg)); }
  .pl-amount { font-size: 1.15rem; }
}
...
.expense-label { color: var(--text); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.expense-bar-wrap {
  height: 8px;
  background: var(--border);
  border-radius: 4px;
...
.expense-amount { text-align: end; font-weight: 700; color: var(--text-strong); }

.pl-total-row {
  background: color-mix(in srgb, var(--primary) 6%, var(--bg));
  font-weight: 800;
  td { border-top: 2px solid var(--border-strong) !important; }
}
...
.reports-header {
  ...
  .quick-dates {
    ...
    button {
      ...
      background: var(--bg-elevated); cursor: pointer; font-size: 0.78rem; font-weight: 700;
...
.kpi-card {
  ...
  &:hover { transform: translateY(-2px); box-shadow: var(--shadow-md, var(--shadow-sm)); }
  .kpi-icon { font-size: 1.8rem; flex-shrink: 0; }
  .kpi-label { font-size: 0.78rem; color: var(--text-muted); margin-bottom: 4px; font-weight: 600; }
  .kpi-value { font-size: 1.15rem; font-weight: 800; color: var(--text-strong); }
  .kpi-sub { font-size: 0.72rem; color: var(--text-muted); margin-top: 3px; }
  &.sales { border-color: color-mix(in srgb, #2e7d4f 30%, var(--border)); background: color-mix(in srgb, #2e7d4f 4%, var(--bg-card)); }
  &.profit { border-color: color-mix(in srgb, #1d6fa4 30%, var(--border)); background: color-mix(in srgb, #1d6fa4 4%, var(--bg-card)); }
  &.expenses { border-color: color-mix(in srgb, #b45309 30%, var(--border)); background: color-mix(in srgb, #b45309 4%, var(--bg-card)); }
  &.inventory { border-color: color-mix(in srgb, #6366f1 30%, var(--border)); background: color-mix(in srgb, #6366f1 4%, var(--bg-card)); }
  &.customers { border-color: color-mix(in srgb, #0891b2 30%, var(--border)); background: color-mix(in srgb, #0891b2 4%, var(--bg-card)); }
  &.danger { border-color: color-mix(in srgb, var(--danger) 30%, var(--border)); background: color-mix(in srgb, var(--danger) 4%, var(--bg-card)); }
  &.cashflow.positive { border-color: color-mix(in srgb, #2e7d4f 30%, var(--border)); }
  &.cashflow.negative { border-color: color-mix(in srgb, var(--danger) 30%, var(--border)); }
}

/* Tables */
.table-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 14px; h3 { margin: 0; } }
.type-filter { display: flex; gap: 4px;
  button { padding: 5px 10px; border: 1px solid var(--border); border-radius: 20px; background: var(--bg); cursor: pointer; font-size: 0.78rem; font-weight: 700; color: var(--text-muted); transition: var(--transition);
...
.report-table { width: 100%; border-collapse: collapse; font-size: 0.88rem;
  th { background: var(--bg); padding: 10px 10px; text-align: right; font-weight: 700; color: var(--text-muted); font-size: 0.8rem; border-bottom: 2px solid var(--border); white-space: nowrap; }
...
.profit-pos { color: #2e7d4f; }
.profit-neg { color: var(--danger); }
.danger-text { color: var(--danger); font-weight: 700; }
.empty { text-align: center; padding: 24px; color: var(--text-muted); font-size: 0.88rem; }

/* Badges */
.type-badge { padding: 3px 8px; border-radius: 20px; font-size: 0.75rem; font-weight: 700;
  &.branch { background: rgba(46,125,79,0.12); color: #2e7d4f; }
  &.wholesale { background: rgba(99,102,241,0.12); color: #4f46e5; }
  &.retail { background: rgba(8,145,178,0.12); color: #0891b2; }
}
.status-badge { padding: 3px 8px; border-radius: 20px; font-size: 0.75rem; font-weight: 700;
  &.success { background: rgba(46,125,79,0.12); color: #2e7d4f; }
  &.warning { background: rgba(180,83,9,0.12); color: #b45309; }
  &.danger { background: rgba(180,35,24,0.12); color: #b42318; }
}

/* Misc */
.mt-4 { margin-top: 4px; }
.loading-state { text-align: center; padding: 48px; color: var(--text-muted); font-size: 1rem; }
.error-msg { color: var(--danger); background: color-mix(in srgb, var(--danger) 8%, transparent); border: 1px solid color-mix(in srgb, var(--danger) 20%, transparent); padding: 10px 14px; border-radius: var(--radius-sm); }

@media (max-width: 1100px) { .kpi-grid { grid-template-columns: repeat(3, minmax(0, 1fr)); } }
@media (max-width: 700px) {
  .kpi-grid, .kpi-grid.kpi-grid-3 { grid-template-columns: 1fr 1fr; }
