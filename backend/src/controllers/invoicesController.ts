...
export const invoices = {
  list: async (req: any, res: any, next: any) => { try { ok(res, await invoiceService.getInvoices(req.query)); } catch (e) { next(e); } },
  create: async (req: any, res: any, next: any) => {
    try { ok(res, await invoiceService.createInvoice(req.body, req.user.id), 'تم إنشاء الفاتورة بنجاح'); } catch (e) { next(e); }
  },
  get: async (req: any, res: any, next: any) => { try { ok(res, await invoiceService.getInvoiceById(req.params.id)); } catch (e) { next(e); } },
  ...
  pdf: async (req: any, res: any, next: any) => {
    try {
  ...
  delete: async (req: any, res: any, next: any) => { try { await invoiceService.deleteInvoice(req.params.id, req.user.id); ok(res, null, 'تم حذف الفاتورة'); } catch (e) { next(e); } },
};
