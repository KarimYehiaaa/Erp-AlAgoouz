...
export interface SaleEntity extends BaseEntity {
  ...
  sale_type: 'wholesale' | 'pos' | 'branch';
  subtotal: number;
  discount_amount: number;
  tax_amount: number;
  total_amount: number;
  payment_status: 'paid' | 'unpaid' | 'partial';
  entry_mode: 'manual' | 'scanner';
  notes?: string;
  sale_date: Date;
  user_id: number;
}
...
export class SalesRepository extends BaseRepository<SaleEntity> {
   ...
   */
  async getSalesList(filters: any = {}): Promise<{ data: any[], meta: any }> {
    ...
    if (filters.sale_type) { sql += ` AND s.sale_type = $${idx++}`; params.push(filters.sale_type); }
    if (filters.entry_mode) { sql += ` AND s.entry_mode = $${idx++}`; params.push(filters.entry_mode); }
    if (filters.from_date) { sql += ` AND s.sale_date >= $${idx++}`; params.push(filters.from_date); }
