...
const renderInvoiceToCanvas = (invoice) => {
  ...
  detailsY += 30;
  ctx.fillText(`التاريخ: ${new Date(invoice.created_at || invoice.sale_date).toLocaleDateString('ar-EG')} ${new Date(invoice.created_at || invoice.sale_date).toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' })}`, rightX, detailsY);
  detailsY += 30;
  ...
  items.forEach((item) => {
    ...
    ctx.fillText(`${Number(item.total_amount || (item.quantity * (item.unit_price || item.sale_price))).toFixed(2)}`, 260, currentY);
  ...
  ctx.fillText(company.tagline || 'شكراً لزيارتكم! نرجو رؤيتكم قريباً', PRINTER_WIDTH / 2, currentY);
...
export const directPrinter = {
  ...
  selectPrinter: async () => {
    try {
      ...
      localStorage.setItem('selected_usb_printer_name', selectedDevice.productName || 'USB Printer');
  ...
  print: async (invoice) => {
    if (!navigator.usb) {
      throw new Error('ميزة الطباعة المباشرة غير مدعومة في هذا المتصفح. يرجى استخدام Chrome أو Edge.');
    }
