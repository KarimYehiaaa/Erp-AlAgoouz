/**
 * run-manual-backup.ts — نسخة احتياطية يدوية موثقة
 * ═══════════════════════════════════════════════════
 * ينفّذ `createBackup` من backupService ويعرض نتيجة مفصلة (اسم الملف
 * والمسار الكامل). يستخدم إعدادات .env للاتصال بقاعدة البيانات.
 *
 * التشغيل: `node scripts/run-manual-backup.ts` (من backend)
 */
