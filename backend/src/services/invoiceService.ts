...
const generateInvoiceNumber = async (client: any) => {
...
export const parseInvoiceData = (data = {}) => {
  ...
  let subtotal = 0;
  const parsedItems = items.map((item: any, idx: any) => {
    if (!item.product_id) {
...
const deductInvoiceInventory = async (client: any, invoiceId: any, items: any, userId: any, invoiceNumber: any) => {
...
const restoreInvoiceInventory = async (client: any, invoiceId: any, userId: any, invoiceNumber: any) => {
...
export const createInvoice = async (data: any, userId: any) => {
...
export const updateInvoice = async (id: any, data: any, userId: any) => {
  const { items: parsedItems, subtotal, discountAmount, taxAmount, totalAmount, paymentStatus } = parseInvoiceData(data);
  const client = await getClient();
  try {
    await client.query('BEGIN');
...
export const deleteInvoice = async (id: any, userId = null) => {
  const client = await getClient();
  try {
    await client.query('BEGIN');
