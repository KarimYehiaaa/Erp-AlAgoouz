 ...
 * @returns {Promise<any>}
 */
export const getHrSummary = async (periodMonth: string) => {
...
/**
 * إنشاء وردية عمل جديدة.
 * @param {Record<string, any>} data بيانات الوردية
 * @returns {Promise<any>}
 */
export const createShift = async (data: Record<string, any>) => {
...
/**
 * جلب قائمة الموظفين مع فلترة وبحث.
 * @param {Record<string, any>} [filters] خيارات الفلترة (active, search...)
 * @returns {Promise<{ rows: any[], total: number }>}
 */
export const listEmployees = async (filters: Record<string, any> = {}) => {
  const params: any[] = [];
  let sql = `
...
/**
 * إضافة موظف جديد.
 * @param {Record<string, any>} data بيانات الموظف
 * @returns {Promise<any>}
 */
...
/**
 * جلب سجلات الحضور مع فلترة.
 * @param {Record<string, any>} [filters] خيارات الفلترة (from_date, to_date, employee_id...)
 * @returns {Promise<any[]>}
 */
export const listAttendance = async (filters: Record<string, any> = {}) => {
  const params: any[] = [];
  let sql = `
...
/**
 * حفظ سجل حضور واحد.
 * @param {Record<string, any>} data بيانات الحضور
 * @param {number} userId معرف المستخدم المنفّذ
 * @returns {Promise<any>}
 */
export const saveAttendance = async (data: Record<string, any>, userId: number) => {
...
/**
 * حفظ حضور لفترة/مجموعة موظفين دفعة واحدة.
 * @param {Record<string, any>} data بيانات النطاق
 * @param {number} userId معرف المستخدم المنفّذ
 * @returns {Promise<any>}
 */
export const saveAttendanceRange = async (data: Record<string, any>, userId: number) => {
  const employeeId = Number(data.employee_id);
  if (!employeeId) throw new AppError('يرجى اختيار الموظف أولاً من القائمة', 400);
...
/**
 * جلب سلف الموظفين مع فلترة.
 * @param {Record<string, any>} [filters] خيارات الفلترة (employee_id, status...)
 * @returns {Promise<any[]>}
 */
export const listAdvances = async (filters: Record<string, any> = {}) => {
  const params: any[] = [];
  let sql = `
...
/**
 * تسجيل سلفة لموظف.
 * @param {Record<string, any>} data بيانات السلفة
 * @param {number} userId معرف المستخدم المنفّذ
 * @returns {Promise<any>}
 */
export const createAdvance = async (data: Record<string, any>, userId: number) => {
...
/**
 * معاينة مسير الرواتب لشهر قبل الاعتماد.
 * @param {string} periodMonth الشهر بصيغة YYYY-MM
 * @returns {Promise<any[]>}
 */
export const previewPayroll = async (periodMonth: string) => {
  const { month } = getMonthBounds(periodMonth);
...
/**
 * إنشاء أو إعادة حساب مسير رواتب لشهر.
 * @param {string} periodMonth الشهر بصيغة YYYY-MM
 * @param {number} userId معرف المستخدم المنفّذ
 * @returns {Promise<any>}
 */
export const createOrRecalculatePayroll = async (periodMonth: string, userId: number) => {
...
/** جلب مسير رواتب واحد بتفاصيله. */
export const getPayrollRun = async (id: number) => {
...
const applyAdvanceDeductions = async (client, employeeId, amount) => {
  ...
  for (const advance of advances.rows) {
    ...
    const installment = roundMoney(advance.installment_amount || advance.amount || 0);
    if (installment <= 0) {
      await client.query(
        `UPDATE employee_advances SET status = 'closed', updated_at = NOW() WHERE id = $1`,
        [advance.id],
      );
      continue;
    }
    const balance = roundMoney(
      Math.max(0, advance.amount - installment * Number(advance.paid_installments || 0)),
    );
    const deduction = Math.min(remaining, balance, installment);
    if (deduction <= 0) continue;
    const newPaidInstallments =
      Math.round((Number(advance.paid_installments || 0) + deduction / installment) * 10000) /
      10000;
    const newBalance = roundMoney(balance - deduction);
    await client.query(
      `UPDATE employee_advances
       SET paid_installments = $1, status = CASE WHEN $2 <= 0.01 THEN 'closed' ELSE status END, updated_at = NOW()
       WHERE id = $3`,
 ...
 * @returns {Promise<any>}
 */
export const payPayrollRun = async (id: number, userId: number, paymentMethod: string = 'cash') => {
  const client = await getClient();
  try {
    await client.query('BEGIN');
    const run = (
...
/**
 * ملخص الحضور (حضور/غياب/إجازات) لفترة.
 * @param {Record<string, any>} [filters] خيارات الفلترة (from_date, to_date...)
 * @returns {Promise<any[]>}
 */
export const getAttendanceSummary = async (filters: Record<string, any> = {}) => {
  const params: any[] = [];
  let sql = `
    SELECT
