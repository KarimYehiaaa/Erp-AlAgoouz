-- ==============================================================================
-- 061_period_lock_delete_protection_and_schema_fixes.sql
-- بن العجوز ERP — تعزيز حماية الفترات المحاسبية ضد الحذف وتوسيع نطاق الجداول المالية
-- ==============================================================================

-- 1. تحديث دالة منع التعديل أو الحذف في الفترات المحاسبية المقفلة
...
BEGIN
  ...
  -- استخراج تاريخ السجل بناءً على نوع الجدول
  IF TG_TABLE_NAME = 'sales' THEN
    record_date := COALESCE(rec.sale_date, rec.created_at::DATE);
  ELSIF TG_TABLE_NAME = 'expenses' THEN
    record_date := COALESCE(rec.expense_date, rec.created_at::DATE);
  ELSIF TG_TABLE_NAME = 'invoices' THEN
    record_date := COALESCE(rec.issued_at::DATE, rec.created_at::DATE);
  ELSIF TG_TABLE_NAME = 'purchase_invoices' THEN
    record_date := COALESCE(rec.invoice_date, rec.created_at::DATE);
  ELSIF TG_TABLE_NAME = 'purchase_returns' THEN
    record_date := COALESCE(rec.return_date, rec.created_at::DATE);
  ELSIF TG_TABLE_NAME = 'journal_entries' THEN
    record_date := COALESCE(rec.entry_date, rec.created_at::DATE);
  ELSIF TG_TABLE_NAME = 'journal_entry_lines' THEN
    ...
    IF record_date IS NULL THEN
      record_date := rec.created_at::DATE;
    END IF;
  ELSE
    record_date := rec.created_at::DATE;
  END IF;
...
-- 2. إعادة تفعيل الـ Trigger بدعم الحذف والتعديل والإدراج لكافة الجداول المالية
DO $$
DECLARE
  tbl TEXT;
...
BEGIN
  FOREACH tbl IN ARRAY financial_tables LOOP
    ...
    -- إنشاء الـ Trigger الجديد المحدث
    EXECUTE format(
