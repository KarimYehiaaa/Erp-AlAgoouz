...
export const suppliers = {
  ...
  payments: async (req, res, next) => { try { ok(res, await supplierService.getSupplierPayments(req.params.id)); } catch (e) { next(e); } },
  recordPayment: async (req, res, next) => { try { ok(res, await supplierService.recordSupplierPayment(req.params.id, req.body, req.user.id), 'تم تسجيل سداد المورد بنجاح'); } catch (e) { next(e); } },
};
