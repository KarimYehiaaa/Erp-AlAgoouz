...
- نتاجات البناء (`frontend/dist/`, `backend/api/`) غير متتبَّعة في Git — الملفات تبقى على القرص وتُبنى محليًا أو عبر CI. `npm run build` يكتب مباشرة إلى `frontend/dist` (الذي يقدّمه الخادم) — لا يُنتج مجلد `dist/` جذر زائد.
