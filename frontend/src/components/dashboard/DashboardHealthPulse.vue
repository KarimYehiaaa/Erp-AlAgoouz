<template>
  <article class="panel health-panel" style="display: flex; flex-direction: column; align-items: center; gap: 14px;">
...
const healthItems = computed(() => [
  ...
  { label: 'تنبيهات المخزون', value: number(props.stats?.stockAlerts), tone: Number(props.stats?.stockAlerts || 0) ? 'danger' : 'success' },
  { label: 'وصفات ناقصة', value: number(props.stats?.recipeSummary?.shortageRecipes), tone: Number(props.stats?.recipeSummary?.shortageRecipes || 0) ? 'warning' : 'success' },
