...
export const useCartStore = defineStore('cart', () => {
  ...
  const customerId = ref<number | string | null>(null);
  const saleType = ref<string>('retail'); // 'retail', 'wholesale', 'delivery'
  const discountAmount = ref<number>(0);
  const taxRate = ref<number>(0); // e.g. 0.14 for 14% VAT if enabled
  ...
  const totalTax = computed(() =>
    Math.max(0, (subtotal.value - totalDiscount.value) * Number(taxRate.value || 0)),
  );
  const grandTotal = computed(() =>
    Math.max(0, subtotal.value - totalDiscount.value + totalTax.value),
  );
