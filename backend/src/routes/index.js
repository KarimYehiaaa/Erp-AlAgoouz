...
router.get(
  '/operations/audit-logs', authenticate, authorize('reports.view'),
...
router.post(
  '/hr/attendance', authenticate, authorize('hr.add'),
...
router.post(
  '/hr/advances', authenticate, authorize('hr.add'),
...
// ─── Branch Sales Excel ───────────────────────────────────────────────────────
router.get(
  '/sales/branch/template', authenticate, authorize('pos.view'),
  api.sales.branchTemplate,
);
router.post(
  '/sales/branch/validate', authenticate, authorize('pos.add'),
  upload.single('file'),
  api.sales.branchValidateExcel,
);
router.post(
  '/sales/branch/import', authenticate, authorize('pos.add'),
  upload.single('file'),
...
router.post(
  '/sales/:id/return', authenticate, authorize('pos.add'),
...
router.delete(
  '/sales/type/:saleType', authenticate, authorize('settings.delete'),
...
router.get(
  '/inventory/movements', authenticate, authorize('inventory.view'),
  api.inventory.movements,
);
router.get(
  '/inventory/return-template', authenticate, authorize('inventory.view'),
  api.inventory.returnTemplate,
);
router.post(
  '/inventory/return-validate', authenticate, authorize('inventory.add'),
  upload.single('file'),
  api.inventory.validateReturnExcel,
);
router.post(
  '/inventory/return-import', authenticate, authorize('inventory.add'),
  upload.single('file'),
  api.inventory.importReturnExcel,
);
router.get('/warehouses', authenticate, api.inventory.warehouses);
router.post(
  '/inventory/transfer', authenticate, authorize('inventory.add'),
  validateBody(inventoryTransferSchema),
  api.inventory.transfer,
);
router.post(
  '/inventory/adjust', authenticate, authorize('inventory.add'),
  validateBody(inventoryAdjustSchema),
  auditLog('inventory_adjust', 'inventory'),
  api.inventory.adjust,
);
router.delete(
  '/inventory', authenticate, authorize('inventory.delete'),
...
router.post(
  '/stocktakes', authenticate, authorize('inventory.add'),
...
router.put(
  '/stocktakes/:id/items', authenticate, authorize('inventory.edit'),
  validateBody(stocktakeUpdateSchema),
  api.stocktake.updateItems,
);
router.post(
  '/stocktakes/:id/complete', authenticate, authorize('inventory.add'),
  auditLog('stocktake_complete', 'inventory'),
  api.stocktake.complete,
);
router.delete(
  '/stocktakes/:id', authenticate, authorize('inventory.delete'),
...
// ─── Purchases ────────────────────────────────────────────────────────────────
router.get(
  '/purchases', authenticate, authorize('products.view'),
  validateQuery(commonQuerySchema),
  api.purchases.list,
);
router.post(
  '/purchases', authenticate, authorize('products.add'),
  validateBody(purchaseInvoiceSchema),
  auditLog('purchase_create', 'purchases'),
  api.purchases.create,
);
router.put(
  '/purchases/:id', authenticate, authorize('products.edit'),
  validateBody(purchaseInvoiceSchema),
  auditLog('purchase_update', 'purchases'),
  api.purchases.update,
);
router.delete(
  '/purchases/:id', authenticate, authorize('products.delete'),
...
router.post(
  '/costs/recipes', authenticate, authorize('products.add'),
  validateBody(recipeSchema),
  api.costs.createRecipe,
);
router.put(
  '/costs/recipes/:id', authenticate, authorize('products.edit'),
  validateBody(recipeSchema),
  api.costs.updateRecipe,
);
router.delete(
  '/costs/recipes/:id', authenticate, authorize('products.delete'),
  api.costs.deleteRecipe,
);
router.post(
  '/costs/recipes/:id/produce', authenticate, authorize('products.add'),
  validateBody(recipeProductionSchema),
  api.costs.produceRecipe,
);
router.get(
  '/costs/productions', authenticate, authorize('products.view'),
  api.costs.listProductions,
);
router.post(
  '/costs/productions/:movementId/reverse', authenticate, authorize('products.add'),
...
// ─── Customers ────────────────────────────────────────────────────────────────
router.get(
  '/customers', authenticate, authorize('pos.view'),
  validateQuery(commonQuerySchema),
  api.customers.list,
);
router.get('/customers/:id', authenticate, authorize('customers.view'), api.customers.get);
router.get(
  '/customers/:id/statement', authenticate, authorize('customers.view'),
  api.customers.statement,
);
router.post(
  '/customers/:id/payment', authenticate, authorize('customers.add'),
  validateBody(paymentSchema),
  api.customers.recordPayment,
);
router.post(
  '/customers/sales/:saleId/payment', authenticate, authorize('customers.add'),
  validateBody(paymentSchema),
  api.customers.recordSalePayment,
);
router.post(
  '/customers', authenticate, authorize('customers.add'),
  validateBody(customerCreateSchema),
  api.customers.create,
);
router.put(
  '/customers/:id', authenticate, authorize('customers.edit'),
...
// ─── Expenses ─────────────────────────────────────────────────────────────────
router.get(
  '/expenses', authenticate, authorize('expenses.view'),
  validateQuery(commonQuerySchema),
  api.expenses.list,
);
router.get(
  '/expenses/categories', authenticate, authorize('expenses.view'),
  api.expenses.categories,
);
router.get(
  '/expenses/report', authenticate, authorize('expenses.view'),
  api.expenses.report,
);
router.post(
  '/expenses', authenticate, authorize('expenses.add'),
  validateBody(expenseSchema),
  auditLog('expense_create', 'expenses'),
  api.expenses.create,
);
router.put(
  '/expenses/:id', authenticate, authorize('expenses.edit'),
  validateBody(expenseUpdateSchema),
  auditLog('expense_update', 'expenses'),
  api.expenses.update,
);
router.delete(
  '/expenses/:id', authenticate, authorize('expenses.delete'),
...
// ─── Suppliers ────────────────────────────────────────────────────────────────
router.get(
  '/suppliers', authenticate, authorize('suppliers.view'),
  validateQuery(commonQuerySchema),
  api.suppliers.list,
);
router.get('/suppliers/:id', authenticate, authorize('suppliers.view'), api.suppliers.get);
router.post(
  '/suppliers', authenticate, authorize('suppliers.add'),
  validateBody(supplierCreateSchema),
  api.suppliers.create,
);
router.put(
  '/suppliers/:id', authenticate, authorize('suppliers.edit'),
...
router.get(
  '/suppliers/:id/invoices', authenticate, authorize('suppliers.view'),
  api.suppliers.invoices,
);
router.get(
  '/suppliers/:id/payments', authenticate, authorize('suppliers.view'),
  api.suppliers.payments,
);
router.post(
  '/suppliers/:id/payments', authenticate, authorize('suppliers.add'),
  validateBody(paymentSchema),
...
// ─── Invoices ─────────────────────────────────────────────────────────────────
router.get(
  '/invoices', authenticate, authorize('pos.view'),
  validateQuery(commonQuerySchema),
  api.invoices.list,
);
router.post(
  '/invoices', authenticate, authorize('invoices.add'),
  validateBody(invoiceSchema),
  api.invoices.create,
);
router.get(
  '/invoices/:id/pdf', authenticate, authorize('pos.view'),
  api.invoices.pdf,
);
router.get(
  '/invoices/:id', authenticate, authorize('pos.view'),
  api.invoices.get,
);
router.put(
  '/invoices/:id', authenticate, authorize('invoices.edit'),
  validateBody(invoiceSchema),
  api.invoices.update,
);
...
// ─── Quotes ───────────────────────────────────────────────────────────────────
router.get(
  '/quotes/template', authenticate, authorize('pos.view'),
  api.quotes.template,
);
router.put(
  '/quotes/template', authenticate, authorize('pos.edit'),
  validateBody(quoteTemplateSchema),
  api.quotes.template,
);
router.post(
  '/quotes/pdf', authenticate, authorize('pos.add'),
...
router.get(
  '/roles/:id/permissions', authenticate, authorize('users.view'),
  api.users.getRolePermissions,
);
router.post(
  '/roles/:id/permissions', authenticate, authorize('users.add'),
...
router.get(
  '/settings', authenticate, authorize('settings.view'),
  api.users.settings,
);
router.put(
  '/settings/:key', authenticate, authorize('settings.edit'),
...
router.get(
  '/forecasting', authenticate, authorize('reports.view'),
  api.forecasting.getDemandForecast,
);
router.get(
  '/forecasting/basket-associations', authenticate, authorize('pos.view'),
  api.forecasting.getBasketAssociations,
);
router.post(
  '/forecasting/copilot', authenticate, authorize('dashboard.add'),
  validateBody(copilotSchema),
  api.forecasting.askCopilot,
);
router.get(
  '/forecasting/staffing', authenticate, authorize('reports.view'),
  api.forecasting.getStaffingForecast,
);
router.get(
  '/forecasting/pricing-alerts', authenticate, authorize('products.view'),
  api.forecasting.getSmartPricingAlerts,
);
router.get(
  '/forecasting/cashflow-projection', authenticate, authorize('reports.view'),
  api.forecasting.getCashFlowProjection,
);
router.get(
  '/expenses/suggest-category', authenticate, authorize('expenses.view'),
...
router.get(
  '/backup/create', authenticate, authorize('settings.view'),
  requireAdmin,
  auditLog('backup_create', 'backup'),
  api.backup.create,
);
router.get(
  '/backup/list', authenticate, authorize('settings.view'),
  requireAdmin,
  api.backup.list,
);
router.get(
  '/backup/download/:name', authenticate, authorize('settings.view'),
  requireAdmin,
  api.backup.download,
);
// BUG-16 FIX: تسجيل audit log لعمليات الاستعادة والحذف الكلي (الأكثر خطورة)
router.post(
  '/backup/restore', authenticate, authorize('settings.add'),
  requireAdmin,
...
router.post(
  '/backup/restore-file', authenticate, authorize('settings.add'),
  requireAdmin,
  upload.single('file'),
  requireConfirmation('CONFIRM_RESTORE_BACKUP'),
...
router.post(
  '/backup/clear', authenticate, authorize('settings.add'),
  requireAdmin,
...
router.post(
  '/backup/cloud-test', authenticate, authorize('settings.add'),
  requireAdmin,
  api.backup.cloudTest,
);
router.get(
  '/backup/logs', authenticate, authorize('settings.view'),
  requireAdmin,
  api.backup.getLogs,
);
