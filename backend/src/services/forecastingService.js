...
import { convertQty, normalizeUnit } from './productCostService.js';
...
export const getDemandForecast = async (filters = {}) => {
  ...
  const ingredientsForecastResult = [];
  products.forEach(prod => {
  ...
  products.forEach(prod => {
    // Current stock
    let currentStock = Number(prod.stock_available || 0);
    ...
    const projectedDaily = ingredientDailyForecasts[prod.id] || forecasts[prod.id] || new Array(forecastDays).fill(0);
