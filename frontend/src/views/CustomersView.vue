  ...
  <div class="customers-page">
    ...
    <div class="page-header">
      <div class="search-wrap">
        ...
        <input v-model="search" type="text" placeholder="بحث بالاسم أو الهاتف أو الكود..." @input="load" class="search-input" />
      </div>
      ...
      <table>
        ...
        <tbody>
          <tr v-for="c in customers" :key="c.id" :class="{ 'has-balance': Number((c.total_balance ?? c.balance) || 0) > 0 }">
            ...
            <td>{{ c.phone || '—' }}</td>
            <td>
              <span class="type-badge" :class="c.customer_type === 'wholesale' ? 'wholesale' : 'retail'">
                {{ c.customer_type === 'wholesale' ? '🏷️ جملة' : '🛍️ تجزئة' }}
              </span>
            </td>
            <td class="amount-cell">{{ formatMoney(c.total_purchased || 0) }}</td>
            <td class="amount-cell" :class="Number((c.total_balance ?? c.balance) || 0) > 0 ? 'balance-due' : 'balance-ok'">
      ...
      <div class="statement-modal">
        ...
        <template v-else-if="statement">
          ...
          <div class="statement-summary">
            ...
            <div class="summary-item balance" :class="statement.summary.total_balance > 0 ? 'due' : 'clear'">
              <div class="si-label">الرصيد المتبقي</div>
              <div class="si-value">{{ formatMoney(statement.summary.total_balance) }}</div>
              <div class="si-sub">{{ statement.summary.total_balance > 0 ? '⚠️ مستحق السداد' : '✅ لا يوجد رصيد' }}</div>
            </div>
          </div>
          ...
          <div class="transactions-section">
            ...
            <div class="transactions-wrap">
              <table class="transactions-table">
                ...
                <tbody>
                  <tr v-for="t in statement.transactions" :key="`${t.entry_type}-${t.id}`"
                    :class="{
                    ...
                    <td>
                      <span class="entry-type" :class="t.entry_type">
                        {{ t.entry_type === 'invoice' ? 'فاتورة' : 'بيع' }}
                      </span>
                    ...
                    <td class="amount-cell" :class="(t.total_amount - t.paid_amount) > 0.01 ? 'balance-due' : 'balance-ok'">
                    ...
                    <td>
                      ...
                      <button
                        v-if="t.entry_type === 'sale' && t.status !== 'cancelled' && (t.total_amount - t.paid_amount) > 0.01"
                        class="pay-sale-btn"
          ...
          <div v-if="activeSalePayment" class="sale-pay-overlay" @click.self="activeSalePayment = null">
            <div class="sale-pay-card">
              ...
              <div class="sale-pay-info">
                <span>إجمالي الفاتورة: <strong>{{ formatMoney(activeSalePayment.total_amount) }}</strong></span>
                <span>المدفوع: <strong class="paid-col">{{ formatMoney(activeSalePayment.paid_amount) }}</strong></span>
                <span>الباقي: <strong class="balance-due">{{ formatMoney(activeSalePayment.total_amount - activeSalePayment.paid_amount) }}</strong></span>
              </div>
      ...
      <div class="card modal-content">
        ...
        <form @submit.prevent="save">
          <div class="form-group">
            ...
            <input v-model="form.code" :placeholder="form.id ? '' : 'اتركه فارغاً للتوليد التلقائي'" />
          </div>
...
// ─── computed ─────────────────────────────────────────────────────────────────
const wholesaleCount = computed(() => customers.value.filter(c => c.customer_type === 'wholesale').length);
const totalBalance = computed(() => customers.value.reduce((s, c) => s + Math.max(0, getCustomerDue(c)), 0));
...
const statusLabel = (s) => ({
  paid: 'مدفوع', unpaid: 'غير مدفوع', partial: 'جزئي', refunded: 'مسترد'
}[s] || s);
...
const submitSalePayment = async () => {
  ...
  try {
    ...
    await load();
    // إغلاق بعد ثانية
    setTimeout(() => { activeSalePayment.value = null; }, 1200);
  } catch (e) {
...
.search-wrap { position: relative; flex: 1; min-width: 220px;
  .search-icon { position: absolute; right: 12px; top: 50%; transform: translateY(-50%); opacity: 0.45; font-size: 0.9rem; pointer-events: none; }
}
.search-input { width: 100%; padding: 10px 36px 10px 12px; border: 1px solid var(--border-strong); border-radius: var(--radius-sm); background: var(--bg-elevated); font-size: 0.9rem; transition: var(--transition);
  &:focus { outline: none; border-color: var(--primary); box-shadow: 0 0 0 3px color-mix(in srgb, var(--primary) 15%, transparent); }
}
.type-filter { padding: 10px 12px; border: 1px solid var(--border-strong); border-radius: var(--radius-sm); background: var(--bg-elevated); font-size: 0.9rem; }

/* Summary */
.summary-row { gap: 14px; }
.summary-card { display: flex; align-items: center; gap: 14px; padding: 16px 20px;
  .summary-icon { font-size: 1.8rem; }
  .summary-label { font-size: 0.8rem; color: var(--text-muted); margin-bottom: 3px; }
  .summary-value { font-size: 1.2rem; font-weight: 800; color: var(--text-strong); }
  &.warn .summary-value { color: var(--warning); }
}

/* Table */
.code-badge { background: var(--bg); padding: 2px 8px; border-radius: 6px; font-size: 0.8rem; border: 1px solid var(--border); }
.name-cell .customer-name { font-weight: 700; }
.type-badge { padding: 3px 10px; border-radius: 999px; font-size: 0.78rem; font-weight: 700;
  &.wholesale { background: color-mix(in srgb, var(--primary) 12%, transparent); color: var(--primary-dark); border: 1px solid color-mix(in srgb, var(--primary) 25%, transparent); }
  &.retail    { background: color-mix(in srgb, var(--accent) 10%, transparent);  color: var(--accent); border: 1px solid color-mix(in srgb, var(--accent) 20%, transparent); }
}
.amount-cell { font-weight: 600; white-space: nowrap; }
.muted { color: var(--text-muted); font-weight: 400; }
.balance-due { color: var(--danger); font-weight: 800; }
.balance-ok  { color: var(--success); }
.has-balance td { background: color-mix(in srgb, var(--warning) 4%, transparent); }
...
/* Statement Modal */
.statement-modal {
  background: var(--bg-card); border-radius: var(--radius-lg); width: min(820px, 96vw);
...
.statement-header {
  ...
  .statement-title { display: flex; align-items: center; gap: 14px;
    .statement-icon { font-size: 2rem; }
    h2 { margin: 0; color: #fff; font-size: 1.2rem; }
    p { margin: 3px 0 0; color: rgba(255,255,255,0.7); font-size: 0.85rem; }
  }
  .close-btn { background: rgba(255,255,255,0.15); border: 1px solid rgba(255,255,255,0.25); color: #fff; width: 32px; height: 32px; border-radius: 50%; cursor: pointer; font-size: 0.9rem; display: flex; align-items: center; justify-content: center; transition: var(--transition);
    &:hover { background: rgba(255,255,255,0.25); }
  }
}

.statement-loading { text-align: center; padding: 48px; color: var(--text-muted); }
...
.summary-item {
  padding: 20px 24px; text-align: center; border-left: 1px solid var(--border);
  &:last-child { border-left: none; }
  .si-label { font-size: 0.82rem; color: var(--text-muted); margin-bottom: 6px; font-weight: 600; }
  .si-value { font-size: 1.4rem; font-weight: 800; }
  .si-sub { font-size: 0.75rem; margin-top: 4px; }
  &.purchased .si-value { color: var(--text-strong); }
  &.paid .si-value { color: var(--success); }
  &.balance.due .si-value { color: var(--danger); }
  &.balance.due { background: color-mix(in srgb, var(--danger) 4%, transparent); }
  &.balance.clear .si-value { color: var(--success); }
  &.balance.clear { background: color-mix(in srgb, var(--success) 4%, transparent); }
}
...
.payment-form {
  ...
  h4 { margin: 0 0 14px; color: var(--primary-dark); font-size: 0.95rem; }
}
.payment-fields { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 12px; }
.payment-actions { display: flex; align-items: center; gap: 14px; margin-top: 12px; }
.pay-msg { font-size: 0.88rem; font-weight: 700;
  &.ok  { color: var(--success); }
  &.err { color: var(--danger); }
}

/* Transactions */
.transactions-section { padding: 16px 20px 20px;
  h4 { margin: 0 0 12px; color: var(--text-strong); font-size: 0.95rem; font-weight: 800; }
}
.transactions-wrap { overflow-x: auto; border-radius: var(--radius-sm); border: 1px solid var(--border); }
.transactions-table {
  width: 100%; border-collapse: collapse; font-size: 0.88rem;
  th { background: var(--bg); padding: 10px 12px; text-align: right; font-weight: 700; color: var(--text-muted); font-size: 0.8rem; border-bottom: 2px solid var(--border); }
  td { padding: 10px 12px; border-bottom: 1px solid var(--border); }
  tr:last-child td { border-bottom: none; }
  .row-sale td { background: color-mix(in srgb, var(--primary) 2%, transparent); }
  .row-invoice td { background: color-mix(in srgb, var(--accent) 2%, transparent); }
  .row-unpaid td { background: color-mix(in srgb, var(--danger) 4%, transparent); }
  .row-partial td { background: color-mix(in srgb, var(--warning) 4%, transparent); }
  .row-cancelled td { opacity: 0.5; }
  .paid-col { color: var(--success); font-weight: 700; }
}
...
.status-badge { padding: 3px 10px; border-radius: 999px; font-size: 0.75rem; font-weight: 700;
  &.paid    { background: color-mix(in srgb, var(--success) 12%, transparent); color: var(--success); }
  &.unpaid  { background: color-mix(in srgb, var(--danger)  12%, transparent); color: var(--danger); }
  &.partial { background: color-mix(in srgb, var(--warning) 12%, transparent); color: var(--warning); }
  &.refunded{ background: color-mix(in srgb, var(--info)    12%, transparent); color: var(--info); }
}
...
/* Pay sale button */
.pay-sale-btn {
  padding: 5px 12px; border-radius: var(--radius-xs); font-size: 0.8rem; font-weight: 700;
  border: 1px solid color-mix(in srgb, var(--success) 35%, transparent);
  background: color-mix(in srgb, var(--success) 10%, transparent);
  color: var(--success); cursor: pointer; transition: var(--transition); white-space: nowrap;
...
.sale-pay-card {
  background: var(--bg-card); border-radius: var(--radius-lg); width: min(440px, 96vw);
  box-shadow: var(--shadow-lg); border: 1px solid var(--border); overflow: hidden;
}
.sale-pay-header {
  display: flex; justify-content: space-between; align-items: center;
  padding: 16px 20px; background: linear-gradient(135deg, var(--success), color-mix(in srgb, var(--success) 70%, #000));
...
.sale-pay-info {
  ...
  background: var(--bg); border-bottom: 1px solid var(--border); font-size: 0.85rem;
  strong { font-weight: 800; }
}
.sale-pay-form { padding: 16px 20px; }
.sale-pay-actions { display: flex; gap: 10px; padding: 0 20px 16px; }

@media (max-width: 768px) {
  .statement-summary { grid-template-columns: 1fr; }
  .payment-fields { grid-template-columns: 1fr; }
  .summary-item { border-left: none; border-bottom: 1px solid var(--border); }
  .sale-pay-info { flex-direction: column; gap: 6px; }
}
</style>
