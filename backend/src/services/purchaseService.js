...
const normalizePurchasePayload = async (client, payload) => {
  ...
  return {
    normalized,
    subtotal,
    invoiceWarehouseId: Number(Object.entries(warehouseCounts).sort((a, b) => b[1] - a[1])[0][0]),
...
export const createPurchaseInvoice = async (payload, userId) => {
  const items = Array.isArray(payload.items) ? payload.items : [];
  if (!items.length) throw new AppError('لا توجد أصناف في الفاتورة', 400);
...
export const updatePurchaseInvoice = async (invoiceId, payload, userId) => {
...
export const deletePurchaseInvoice = async (invoiceId, userId) => {
  const id = Number(invoiceId);
  if (!id) throw new AppError('رقم الفاتورة غير صحيح', 400);
